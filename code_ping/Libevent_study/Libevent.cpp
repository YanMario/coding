...
void OnAccept(int sockfd, short iEvent, void *arg)
{
    ...
    //连接
    // struct event *event_read = new event;
    // event_set(event_read, c, EV_READ|EV_PERSIST, OnRead, NULL);
    ...
    bufferevent_setcb(logic_pro_bev, OnRead, NULL, bufferent_event_erro3, NULL);
...
int main()
{
    ...
    // struct event even_listen;
    struct event *even_listen = event_new(base, sockfd, EV_READ|EV_PERSIST, OnAccept, (void*)base);
    //设置事件
    // event_set(even_listen, sockfd, EV_READ|EV_PERSIST, OnAccept, base);

    //设置base事件
    // event_base_set(base, even_listen);
